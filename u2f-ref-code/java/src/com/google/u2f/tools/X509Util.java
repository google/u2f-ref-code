...
public class X509Util {
  ...
  public static X509Certificate[] parseCertificateChain(DataInputStream encodedDerCertificates) {
    ...
    try {
      while (encodedDerCertificates.available() > 0) {
        // Note: CertificateFactory#generateCertificate will extract one certificate from a byte
        // stream even when there are trailing bytes remaining. Relying on this allows us to
        // repeatedly call this method over a byte stream containing concatenated certificates.
        certChain.add((X509Certificate) CertificateFactory.getInstance(X509).generateCertificate(
            encodedDerCertificates));
        Log.info("Parsed cert: " + certChain.get(certChain.size() - 1));
      }
    } catch (IOException | CertificateException e) {
      // no more certs to read
      Log.info("No more certs");
    }
   ...
   *   {@code certChain[i+1]}.  Finally, {@code certChain[certChain.length-1]} should be signed
    ...
    for (int i = 0; i < caCerts.length; i++) {
      try {
        certChain[certChain.length - 1].verify(caCerts[i].getPublicKey());
          ...
          | NoSuchAlgorithmException | CertificateException e) {
        Log.log(Level.INFO, "Cert chain validation failed to match a root.", e);
  ...
  /**
   * Encodes an array of certificates into a byte array where all the certificates are DER encoded
